        ...
        for line in lines:
            if line.strip():
                ...
                if len(parts) > 1:
                    parts = [parts[-1]]
                if parts:
                    candidate = os.path.join(repo_root, parts[0].strip('"'))
    ...
    try:
        if ext == '.js':
            ...
            with open(file_path, 'r', encoding='utf-8') as source_file:
                ast.parse(source_file.read(), filename=file_path)
...
def check_markdown_codeblocks(file_path):
    ...
    if not ('.codex/' in normalized_lower or '/docs/planning_' in normalized_lower):
        return []
...
def generate_qa_steps(changed_files):
    return [
        ...
        "Chạy kiểm tra cú pháp/kiểm thử nhanh phù hợp với loại file.",
        "Xác nhận thay đổi đáp ứng yêu cầu và không tạo file rác trong repo.",
    ]
